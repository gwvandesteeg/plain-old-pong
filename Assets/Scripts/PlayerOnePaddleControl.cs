...
namespace PaddleGame {
	...
	/**
	 * The controls and instantiation of PlayerOne's paddle based upon the prefab
	 * returned by the GameConfiguration singletons getPlayerOnePaddle method
	 * 
	 * @author	Gerwin van de Steeg
	 *
	 */
	public class PlayerOnePaddleControl : MonoBehaviour, IUserInput {
		//! the Input proxy
		...
		void Awake () {
			...
			// grab our prefab
			paddlePrefab = GameConfiguration.singleton.getPlayerOnePaddle ();
			CreatePaddle ();
			SetupMaterial ();
			SetupForPlayerOne ();
